...
public class CategoryTranslationController {
    ...
    @GetMapping
    public ResponseEntity<List<Translation>> getCategoryTranslations(@PathVariable("wikiName") String wikiName, @PathVariable("categoryName") String categoryName) {
        ...
        List<Translation> translations = this.translationService.findByTranslatableTypeAndTranslatableId(EnumTranslatableType.CATEGORY, opt_category.get().getId());
        return ResponseEntity.ok().body(translations);
    }
    
    @GetMapping("/{locale}")
    public ResponseEntity<Translation> getCategoryTranslation(@PathVariable("wikiName") String wikiName, @PathVariable("categoryName") String categoryName, @PathVariable("locale") String locale) {
        Optional<Wiki> opt_wiki = this.wikiService.findByName(wikiName);
        if(opt_wiki.isEmpty())
            return ResponseEntity.notFound().build();
        ...
        Optional<Translation> translation = this.translationService.findByTranslatableTypeAndTranslatableIdAndLocale(EnumTranslatableType.CATEGORY, opt_category.get().getId(), locale);
    ...
    @PostMapping
    public ResponseEntity<?> createTranslation(@AuthenticationPrincipal User requester, @PathVariable("wikiName") String wikiName, @PathVariable("categoryName") String categoryName, @Valid @RequestBody CreateTranslationRequest request) {
        Optional<Wiki> opt_wiki = this.wikiService.findByName(wikiName);
        if(opt_wiki.isEmpty())
            return ResponseEntity.notFound().build();
        ...
        request.setType(EnumTranslatableType.CATEGORY);
        request.setId(opt_category.get().getId());
    ...
    @PutMapping("/{locale}")
    public ResponseEntity<?> updateTranslation(@AuthenticationPrincipal User requester, @PathVariable("wikiName") String wikiName, @PathVariable("categoryName") String categoryName, @PathVariable("locale") String locale, @Valid @RequestBody UpdateTranslationRequest request) {
        Optional<Wiki> opt_wiki = this.wikiService.findByName(wikiName);
        if(opt_wiki.isEmpty())
            return ResponseEntity.notFound().build();
